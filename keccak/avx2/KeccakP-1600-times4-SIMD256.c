...
#define LOAD4_64(a, b, c, d)                                                   \
  _mm256_set_epi64x((uint64_t)(a), (uint64_t)(b), (uint64_t)(c), (uint64_t)(d))
...
#define SHUFFLE64(a, b, c)                                                     \
  _mm256_castpd_si256(                                                         \
      _mm256_shuffle_pd(_mm256_castsi256_pd(a), _mm256_castsi256_pd(b), c))
...
void KeccakP1600times4_AddLanesAll(void *states, const unsigned char *data,
  ...
  const uint64_t *curData1 =
      (const uint64_t *)(data + laneOffset * SnP_laneLengthInBytes);
  const uint64_t *curData2 =
      (const uint64_t *)(data + laneOffset * 2 * SnP_laneLengthInBytes);
  const uint64_t *curData3 =
      (const uint64_t *)(data + laneOffset * 3 * SnP_laneLengthInBytes);
  V256 lanes0, lanes1, lanes2, lanes3, lanesL01, lanesL23, lanesH01, lanesH23;

#define Xor_In(argIndex)                                                       \
  XOReq256(stateAsLanes[argIndex],                                             \
           LOAD4_64(curData3[argIndex], curData2[argIndex],                    \
                    curData1[argIndex], curData0[argIndex]))
...
#define Xor_In4(argIndex)                                                      \
  ...
  lanes3 = LOAD256u(curData3[argIndex]), INTLEAVE(),                           \
...
void KeccakP1600times4_OverwriteBytes(void *states, unsigned int instanceIndex,
  ...
  if (sizeLeft > 0) {
    memcpy(&statesAsLanes[laneIndex(instanceIndex, lanePosition)], curData,
           sizeLeft);
  }
}
...
void KeccakP1600times4_ExtractBytes(const void *states,
  ...
  if (sizeLeft > 0) {
    memcpy(curData, &statesAsLanes[laneIndex(instanceIndex, lanePosition)],
           sizeLeft);
  }
}
...
void KeccakP1600times4_ExtractLanesAll(const void *states, unsigned char *data,
  ...
  uint64_t *curData1 =
      (uint64_t *)(data + laneOffset * 1 * SnP_laneLengthInBytes);
  uint64_t *curData2 =
      (uint64_t *)(data + laneOffset * 2 * SnP_laneLengthInBytes);
  uint64_t *curData3 =
      (uint64_t *)(data + laneOffset * 3 * SnP_laneLengthInBytes);
...
#define Extr(argIndex)                                                         \
  curData0[argIndex] = stateAsLanes64[4 * (argIndex)],                         \
...
#define Extr4(argIndex)                                                        \
  ...
  lanes3 = LOAD256(stateAsLanes[argIndex + 3]), UNINTLEAVE(),                  \
...
void KeccakP1600times4_ExtractAndAddLanesAll(const void *states,
  ...
  const uint64_t *curInput1 =
      (uint64_t *)(input + laneOffset * 1 * SnP_laneLengthInBytes);
  const uint64_t *curInput2 =
      (uint64_t *)(input + laneOffset * 2 * SnP_laneLengthInBytes);
  const uint64_t *curInput3 =
      (uint64_t *)(input + laneOffset * 3 * SnP_laneLengthInBytes);
  uint64_t *curOutput0 = (uint64_t *)output;
  uint64_t *curOutput1 =
      (uint64_t *)(output + laneOffset * 1 * SnP_laneLengthInBytes);
  uint64_t *curOutput2 =
      (uint64_t *)(output + laneOffset * 2 * SnP_laneLengthInBytes);
  uint64_t *curOutput3 =
      (uint64_t *)(output + laneOffset * 3 * SnP_laneLengthInBytes);
...
#define ExtrXor(argIndex)                                                      \
  ...
  curOutput1[argIndex] =                                                       \
      curInput1[argIndex] ^ stateAsLanes64[4 * (argIndex) + 1],                \
  curOutput2[argIndex] =                                                       \
      curInput2[argIndex] ^ stateAsLanes64[4 * (argIndex) + 2],                \
  curOutput3[argIndex] =                                                       \
      curInput3[argIndex] ^ stateAsLanes64[4 * (argIndex) + 3]
...
#define ExtrXor4(argIndex)                                                     \
  ...
  lanesH23 = LOAD256u(curInput3[argIndex]), XOReq256(lanes0, lanesL01),        \
  XOReq256(lanes1, lanesH01), XOReq256(lanes2, lanesL23),                      \
  XOReq256(lanes3, lanesH23), STORE256u(curOutput0[argIndex], lanes0),         \
...
void KeccakP1600times4_PermuteAll_6rounds(void *states) {
  ...
  copyFromState(A, statesAsLanes) rounds6 copyToState(statesAsLanes, A)
}
...
void KeccakP1600times4_PermuteAll_4rounds(void *states) {
  ...
  copyFromState(A, statesAsLanes) rounds4 copyToState(statesAsLanes, A)
}
...
#undef Xor_In4
    ...
    const uint64_t *curData1 =
        (const uint64_t *)(data +
                           laneOffsetParallel * 1 * SnP_laneLengthInBytes);
    const uint64_t *curData2 =
        (const uint64_t *)(data +
                           laneOffsetParallel * 2 * SnP_laneLengthInBytes);
    const uint64_t *curData3 =
        (const uint64_t *)(data +
                           laneOffsetParallel * 3 * SnP_laneLengthInBytes);
    V256 *statesAsLanes = (V256 *)states;
    declareABCDE

    copyFromState(A, statesAsLanes) while (
        dataByteLen >= (laneOffsetParallel * 3 + laneCount) * 8) {
#define XOR_In(Xxx, argIndex)                                                  \
  XOReq256(Xxx, LOAD4_64(curData3[argIndex], curData2[argIndex],               \
                         curData1[argIndex], curData0[argIndex]))
      XOR_In(Aba, 0);
      XOR_In(Abe, 1);
      XOR_In(Abi, 2);
...
#undef XOR_In
    ...
    copyToState(statesAsLanes, A) return (const unsigned char *)curData0 -
        dataStart;
#endif
  } else {
    const unsigned char *dataStart = data;
    ...
    while (dataByteLen >= (laneOffsetParallel * 3 + laneCount) * 8) {
      KeccakP1600times4_AddLanesAll(states, data, laneCount,
                                    laneOffsetParallel);
      KeccakP1600times4_PermuteAll_24rounds(states);
...
#define LoadXOReq256(lanes, inp, argIndex)                                     \
  XOReq256(lanes, LOAD4_64(inp[3 * 25 + argIndex], inp[2 * 25 + argIndex],     \
                           inp[1 * 25 + argIndex], inp[0 * 25 + argIndex]))
...
#define AddOverWr4(lanes0, lanes1, lanes2, lanes3, key, inp, argIndex)         \
  lanes0 = _mm256_i32gather_epi64((const long long int *)&inp[argIndex + 0],   \
                                  gather, 1),                                  \
  lanes1 = _mm256_i32gather_epi64((const long long int *)&inp[argIndex + 1],   \
                                  gather, 1),                                  \
  lanes2 = _mm256_i32gather_epi64((const long long int *)&inp[argIndex + 2],   \
                                  gather, 1),                                  \
  lanes3 = _mm256_i32gather_epi64((const long long int *)&inp[argIndex + 3],   \
                                  gather, 1),                                  \
...
#define ExtrAccu(lanes, p, argIndex)                                           \
  p[argIndex] ^=                                                               \
      _mm256_extract_epi64(lanes, 0) ^ _mm256_extract_epi64(lanes, 1) ^        \
      _mm256_extract_epi64(lanes, 2) ^ _mm256_extract_epi64(lanes, 3)

#define ExtrAccu4(lanes0, lanes1, lanes2, lanes3, p, argIndex)                 \
  UNINTLEAVEa(lanes0, lanes1, lanes2, lanes3), XOReq256(lanes0, lanes1),       \
      XOReq256(lanes2, lanes3),                                                \
      lanes1 = LOAD256(p[argIndex]), XOReq256(lanes0, lanes2),                 \
      XOReq256(lanes0, lanes1), STORE256(p[argIndex], lanes0)

#define Kravatte_Rollc()                                                       \
  Asa = x0x1x2x3, Ase = x1x2x3x4, ROL64in256(x1x2x3x4, x0x1x2x3, 7),           \
  XOReq256(x1x2x3x4, Ase), XOReq256(x1x2x3x4, _mm256_srli_epi64(Ase, 3)),      \
  Asi = _mm256_blend_epi32(_mm256_permute4x64_epi64(Ase, 0x39),                \
                           _mm256_permute4x64_epi64(x1x2x3x4, 0x39), 0xC0),    \
  Aso = PERM128(Ase, x1x2x3x4, 0x21),                                          \
  Asu = _mm256_blend_epi32(_mm256_permute4x64_epi64(Ase, 0xFF),                \
                           _mm256_permute4x64_epi64(x1x2x3x4, 0x90), 0xFC),    \
  x0x1x2x3 = Asu

ATTRIBUTE_TARGET_AVX2
size_t KeccakP1600times4_KravatteCompress(uint64_t *xAccu, uint64_t *kRoll,
                                          const unsigned char *input,
                                          size_t inputByteLen) {
  ...
  do {
    ...
    rounds6 ExtrAccu4(Aba, Abe, Abi, Abo, xAccu, 0);
...
#define ExtrAddKey4(lanes0, lanes1, lanes2, lanes3, p, argIndex)               \
  XOReq256(lanes0, CONST256_64(kRoll[argIndex + 0])),                          \
      ...
      _mm256_i32scatter_epi64((long long int *)&p[argIndex + 0], scatter,      \
                              lanes0, 1),                                      \
      _mm256_i32scatter_epi64((long long int *)&p[argIndex + 1], scatter,      \
                              lanes1, 1),                                      \
      _mm256_i32scatter_epi64((long long int *)&p[argIndex + 2], scatter,      \
                              lanes2, 1),                                      \
      _mm256_i32scatter_epi64((long long int *)&p[argIndex + 3], scatter,      \
                              lanes3, 1)
...
ATTRIBUTE_TARGET_AVX2
size_t KeccakP1600times4_KravatteExpand(uint64_t *yAccu, const uint64_t *kRoll,
                                        unsigned char *output,
  ...
  do {
    ...
    Asi = _mm256_insert_epi64(_mm256_permute4x64_epi64(Ase, 0x39),
                              _mm256_extract_epi64(lanesL01, 0), 3);
    Aso = _mm256_permute2x128_si256(Ase, lanesL01, 0x21);
    Asu = _mm256_insert_epi64(_mm256_permute4x64_epi64(lanesL01, 0x93),
                              _mm256_extract_epi64(Ase, 3), 0);
    ...
    rounds6 ExtrAddKey4(Aba, Abe, Abi, Abo, out64, 0);
